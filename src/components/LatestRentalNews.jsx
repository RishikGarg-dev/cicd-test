...
export default function LatestRentalNews({ news = [], loading = false, error = null }) {
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 pb-12">
        <h2 className="text-3xl font-bold mb-6 text-black flex items-center gap-2">
          <span className="w-2 h-8 bg-black rounded-full inline-block" />
          Latest Rental Market News
        </h2>
        <div className="flex gap-6 overflow-x-auto scrollbar-hide pb-2">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="w-[340px] flex-shrink-0 bg-white rounded-3xl border border-zinc-100 h-[400px] shadow-sm relative overflow-hidden">
              {/* Shimmer effect */}
  ...
  if (error) {
    return (
      <div className="max-w-7xl mx-auto px-6 pb-12">
        ...
        <div className="bg-white border border-black text-black p-8 rounded-3xl flex flex-col items-center text-center shadow-lg shadow-zinc-100">
          <div className="w-16 h-16 bg-zinc-100 rounded-2xl flex items-center justify-center shadow-sm mb-4 border border-black">
  ...
  if (news.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-6 pb-12">
        ...
        <div className="bg-white border border-zinc-100 text-black p-12 rounded-3xl text-center shadow-lg shadow-zinc-50">
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pb-12">
      ...
      <div className="flex gap-6 overflow-x-auto scrollbar-hide pb-6 pt-2">
        {news.slice(0, 8).map((article, index) => (
          <div
            key={index}
            className="group w-[340px] flex-shrink-0 bg-white rounded-3xl border border-zinc-100 shadow-sm hover:shadow-xl hover:shadow-zinc-200 transition-all duration-300 overflow-hidden flex flex-col hover:-translate-y-2"
          >
            ...
            <div className="p-5 flex flex-col flex-1">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] font-bold text-white px-2 py-0.5 bg-black rounded uppercase tracking-wider">
                  {article.source.name}
                </span>
              ...
              <h3 className="font-bold text-lg mb-2 text-black line-clamp-2 h-[56px] leading-snug group-hover:text-zinc-700 transition-colors">
                {article.title}
              </h3>
              
              <p className="text-sm text-zinc-600 line-clamp-3 h-[60px] mb-4 leading-relaxed">
                {article.description}
              </p>
              ...
              <div className="mt-auto pt-4 border-t border-zinc-100 flex items-center justify-between">
                <Link
                  to={`/news/${index}`}
                  state={{ article }}
                  className="flex items-center gap-2 text-black font-bold text-sm group/link underline underline-offset-4 decoration-zinc-200 hover:decoration-black transition-all"
                >
                  Read Article
                ...
                <div className="w-8 h-8 rounded-full bg-zinc-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border border-zinc-100">
